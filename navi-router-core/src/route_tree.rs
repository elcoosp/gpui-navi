...
impl Segment {
    pub fn param_name(&self) -> Option<&str> {
        match self {
            Segment::Dynamic { name, .. } | Segment::Optional { name, .. } => Some(name),
            _ => None,
        }
    }
    ...
    fn parse_segments(pattern: &str) -> Vec<Segment> {
        ...
        for part in parts {
            if part.starts_with('[') && part.ends_with(']') {
                ...
                let suffix = if close + 1 < part.len() {
                    Some(part[close + 1..].to_string())
                } else {
                    None
                };
...
impl RouteTrie {
    ...
    fn match_segments<'a>(
        ...
        if let Some(child) = node.children.get(seg) {
            if let Some(r) = self.match_segments(rest, child) {
                return Some(r);
            }
        }
        for (key, child) in &node.children {
            if let Some(name) = key.strip_prefix(':') {
                ...
                if let Some((sub, route)) = self.match_segments(rest, child) {
                    params.extend(sub);
        ...
        for (key, child) in &node.children {
            if let Some(opt) = key.strip_prefix('?') {
                ...
                p1.insert(opt.to_string(), seg.to_string());
                if let Some((sub, route)) = self.match_segments(rest, child) {
                    p1.extend(sub);
                    return Some((p1, route));
                }
                if let Some((sub, route)) = self.match_segments(segments, child) {
                    return Some((sub, route));
                }
            }
        }
...
impl RouteTree {
    ...
    pub fn ancestors(&self, route_id: &str) -> Vec<&RouteNode> {
        ...
        while let Some(id) = cur {
            if let Some(node) = self.nodes.get(&id) {
                chain.push(node);
                if chain.len() > 100 {
                    log::error!("Cycle detected at {}", route_id);
                    break;
                }
    ...
    pub fn add_route(&mut self, node: RouteNode) {
        ...
        if let Some(p) = &node.parent {
            self.children.entry(p.clone()).or_default().push(id.clone());
        }
    ...
    pub fn children_of(&self, pid: &str) -> Option<&Vec<String>> {
...
mod tests {
    ...
    fn make_node(id: &str, pat: &str) -> RouteNode {
        RouteNode {
            id: id.into(),
            pattern: RoutePattern::parse(pat),
            parent: None,
            is_layout: false,
            is_index: false,
            has_loader: false,
    ...
    fn test_static() {
        ...
        t.add_route(make_node("u", "/users"));
        let (_p, r) = t.match_path("/users").unwrap();
        assert_eq!(r.id, "u");
    }
    #[test]
    fn test_dyn() {
        let mut t = RouteTree::new();
        t.add_route(make_node("d", "/users/$id"));
        let (p, n) = t.match_path("/users/42").unwrap();
        assert_eq!(n.id, "d");
    ...
    fn test_splat() {
        let mut t = RouteTree::new();
        t.add_route(make_node("s", "/docs/$"));
        let (p, n) = t.match_path("/docs/a/b").unwrap();
        assert_eq!(n.id, "s");
        assert_eq!(p.get("*splat").unwrap(), "a/b");
    }
    #[test]
    fn test_opt_consume() {
        let mut t = RouteTree::new();
        t.add_route(make_node("o", "/{-$id}"));
        let (p, n) = t.match_path("/42").unwrap();
        assert_eq!(n.id, "o");
        assert_eq!(p.get("id").unwrap(), "42");
    }
    #[test]
